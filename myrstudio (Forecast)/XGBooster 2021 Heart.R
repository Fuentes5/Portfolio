#New Code___________________________
...
#Import the data
mydata=read.csv("C:/Users/Student/Documents/myrstudio/rawdata/heart.csv")
...
mydata$myresponse=mydata$HeartDisease #Substitute "Status" with the name of your response variable
mydata$HeartDisease=NULL #Substitute "Status" with the name of your response variable
...
mydata$ChestPainType=as.factor(mydata$ChestPainType)
mydata$FastingBS=as.factor(mydata$FastingBS)
mydata$RestingECG=as.factor(mydata$RestingECG)
mydata$ExerciseAngina=as.factor(mydata$ExerciseAngina)
mydata$ST_Slope=as.factor(mydata$ST_Slope) 
...
#New Code______________________________________________
param.set <- makeParamSet(
  ...
  makeIntegerParam("early_stopping_rounds", lower = 5, upper = 50)
)
